    ...
    public class ContestModels
    {
        ...
        public class Contest
        {
            ...
            public DataTable GetAllContests()
            {
                PostgresModels Database = new PostgresModels();
                DataTable dt = new DataTable("data");
                dt = Database.SqlQuery("SELECT contests.name AS \"Namn\", timestart AS \"Start\", timeend \"Slut\", maxplayers AS \"Max antal spelare\", closetime AS \"Sista anm.dag\", contests.id FROM contests LEFT JOIN reservations ON contests.reservationid = reservations.id WHERE timestart > CURRENT_DATE AND closetime > CURRENT_DATE AND publish = false", PostgresModels.list = new List<NpgsqlParameter>()
                {
                    //new NpgsqlParameter("@time", DateTime.Now)
                });
